      ...
      >
        <table className={MODULE_TABLE_LAYOUT_CLASS} style={{ minWidth: DOC_TABLE_MIN_WIDTH }}>
          <thead className={MODULE_TABLE_THEAD_CLASS}>
            <tr>
              <th
                className={`relative min-w-[300px] w-[300px] border-b border-border-default ${DOC_TABLE_DOCUMENT_COL_CLASS} py-3 text-left align-middle text-sm font-medium text-text-secondary sticky left-0 ${MODULE_TABLE_TH_STICKY_EDGE_CLASS} ${showLeftStickyEdge ? 'shadow-[2px_0_8px_-2px_rgba(0,0,0,0.08)]' : ''}`}
              >
                {showLeftStickyEdge ? (
              ...
              <th className={`${MODULE_TABLE_TH_SCROLL_CLASS} ${MODULE_TABLE_SUMMARY_COL_CLASS} ${DOC_TABLE_SUMMARY_COL_CLASS} py-3 text-left align-middle text-sm font-medium text-text-secondary`}>
              ...
              <th
                className={`relative border-b border-border-default py-3 pl-2 pr-2 text-left align-middle text-sm font-medium text-text-secondary ${moduleTableStatusStickyRightClass(64)} ${MODULE_TABLE_TH_STICKY_EDGE_CLASS} ${showRightStickyEdge ? 'shadow-[-2px_0_8px_-2px_rgba(0,0,0,0.08)]' : ''}`}
              >
                {showRightStickyEdge ? (
            ...
            {rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <td
                    className={`relative border-b border-border-default py-3 pl-2 pr-2 align-top ${moduleTableStatusStickyRightClass(64)} z-[6] ${cellSurface} ${showRightStickyEdge ? 'shadow-[-2px_0_8px_-2px_rgba(0,0,0,0.08)]' : ''}`}
                  >
                    {showRightStickyEdge ? (
                      <span className="pointer-events-none absolute left-[-1px] top-0 z-[8] h-full w-px bg-[#dbdee1]/60" />
                    ) : null}
