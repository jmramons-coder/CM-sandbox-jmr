...
export const HARD_CODED_SURFACE_INVENTORY: HardcodedSurfaceFinding[] = [
  ...
  },
  {
    ...
    area: 'Demo data',
    files: ['src/app/data/mock-cases.ts', 'src/app/data/mock-tasks.ts'],
    issue: 'Fixture content is almost entirely Income Protection claim narrative; canonical data lives in sbli-dataset.',
    target: 'Move to named datasets with coherent cross-workflow object graphs. See docs/LEGACY_MOCK_INVENTORY.md.',
  },
  {
    id: 'assistant-hardcoded-claim-replies',
    category: 'assistant_content',
    area: 'Copilot replies',
    files: [
      'src/app/data/sbli-assistant-responses.ts',
      'src/app/domain/assistantReplyBuilder.ts',
      'src/app/components/caseInsightsData.ts',
    ],
    issue: 'Residual UK IP copy may exist in overlay-only paths or e-app.',
    target: 'Dataset assistantResponses + dynamic workload builders; see docs/LEGACY_MOCK_INVENTORY.md.',
  },
  {
